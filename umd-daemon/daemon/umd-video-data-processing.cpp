...
public:
  H264DataProcessor(UmdGadget *gadget,C2Module *C2Module, std::shared_ptr<Camera3DeviceClient> deviceClient,
      UmdVideoSetup setup) {
    mGadget = gadget;
    mC2Module = C2Module;
...
private:
  ...
  void processData(StreamBuffer buffer) override {
    ...
    UMD_LOG_DEBUG("%s: Generated key for frame no %d and stream_id %d is %d",__func__,
        buffer.frame_number, buffer.stream_id, id);
    MemAllocFlags usage;
    MemAllocError ret;
...
public:
  MjpegDataProcessor(UmdGadget *gadget, std::shared_ptr<Camera3DeviceClient> deviceClient) {
    mGadget = gadget;
    mDeviceClient = deviceClient;
  }
...
public:
  Yuy2DataProcessor(UmdGadget *gadget, std::shared_ptr<Camera3DeviceClient> deviceClient) {
    mGadget = gadget;
    mDeviceClient = deviceClient;
  }
...
UmdVideoData *UmdVideoDataFactory ::createUmdVideoInstance(UmdVideoSetup mVsetup,
                                                           UmdGadget *gadget,
                                                           C2Module *C2Module,
                                                           std::shared_ptr<Camera3DeviceClient> deviceClient) {
